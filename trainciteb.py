...
print(len(lista),len(listb),len(listc),len(listd),len(liste),len(listf),len(listg))
print(s/7)    
l=[]
for i in labels:
    if(i==0):
...
def train(epoch):
    ...
    loss_train = F.cross_entropy(output[idx_train], labels[idx_train], weight=weight) + 0.2*reg
